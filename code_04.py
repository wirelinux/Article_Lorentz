...
def simular(CI):
    ...
    for i in range(trajetoria.shape[0]):
        ...
        z_atual = X[2]
        if (z_prev < z_plano and z_atual >= z_plano) or (
            z_prev > z_plano and z_atual <= z_plano):
...
for i, pontos in enumerate(resultados):
    if len(pontos) > 0:
        plt.scatter(pontos[:,0], pontos[:,1], s=3, alpha=0.9, color=cores[i],
                    label=f'C.I. {i+1}')
